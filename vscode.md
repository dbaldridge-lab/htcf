...
- A list of detected shells will appear at the top of the screen. Select Git Bash (for Windows users) or bash from the list.
...
⚠️ Please be mindful that VS Code extensions can consume significant resources and will run on the HTCF login node upon connecting. Only enable extensions that you will actively use during the session. In general, software development should be performed locally when possible.
...
Enter the following at the prompt of a local terminal (i.e. NOT in a window where you are connected to HTCF):
```
